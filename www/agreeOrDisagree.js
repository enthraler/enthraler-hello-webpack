...
AgreeOrDisagree.prototype = {
	...
	,render: function(plainJsonData) {
		...
		var groups = [];
		var _g = 0;
		var _g1 = this.authorData.responses;
		while(_g < _g1.length) {
			var respondant = _g1[_g];
			++_g;
	...
	,drawTheDots: function() {
		...
		this.nodes = js_d3_D3.range(this.get_numberOfNodes()).map(function(index) {
			...
			return { responseIndex : index, radius : Math.sqrt(v) * _gthis.maxRadius, color : color(i), cx : _gthis.xScale(i), cy : _gthis.height / 2};
		});
		this.force = js_d3_D3.layout.force().nodes(this.nodes).size([this.width,this.height]);
		var svg = js_d3_D3.select("body").append("svg").attr("width",this.width).attr("height",this.height);
		this.circle = svg.selectAll("circle").data(this.nodes).enter().append("circle").attr("r",function(d) {
			return d.radius;
	...
	,showQuestion: function(questionIndex) {
		...
		switch(_g[1]) {
		case 0:
			console.log("set getresponse for GroupedAnswer");
			...
			while(_g1 < _g11.length) {
				...
				++_g1;
				var response1 = respondant[questionIndex];
				addGroup(response1);
			}
			break;
		case 1:
			var groups = _g[2];
			console.log("set getresponse for GroupedWeightedAnswer");
	...
	,updateNodes: function() {
		var _gthis = this;
		this.setNumberOfGroups(Math.ceil(Math.random() * 5));
		console.log("update nodes");
		this.nodes = this.nodes.map(function(node) {
			var i = Math.floor(Math.random() * _gthis.numberOfClusters);
			var v = (i + 1) / _gthis.numberOfClusters * -Math.log(Math.random());
			node.radius = Math.sqrt(v) * _gthis.maxRadius;
			node.cx = _gthis.xScale(i);
			return node;
		});
		this.force.resume();
	}
